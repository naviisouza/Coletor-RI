    ...
    public static List<Application.NewsData> extractNewsData(String url, int maxNews) {
        ...
        try {
            ...
            for (Element link : newsLinks) {
                ...
                if (!title.isEmpty()) {
                    newsList.add(new Application.NewsData("G1", title, newsUrl));
                    count++;
                }
                if (count >= maxNews) {
                    break;
            ...
            System.out.println("Notícias do G1 extraídas com sucesso!");
        } catch (IOException e) {
            // Trata erros de conexão ou leitura de página
            System.err.println("Erro ao tentar acessar a página do G1: " + e.getMessage());
        }
        return newsList;
    }

    // Extrai os links das notícias principais da página inicial do G1
    private static Elements extractNewsLinks(Document document) {
        return document.select("a[href*='/noticia/']");
    }
}
